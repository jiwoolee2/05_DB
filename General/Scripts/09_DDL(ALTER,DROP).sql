...
--> 이미 한국으로 되어있어서 대한민국으로 기본값 변경 불가
SELECT * FROM DEPT_COPY;



-- LNAME 컬럼 값을 기본값으로 수정 !!  --> UPDATE 써야함!!!!!-> 값을 바꿀 떄
...
-- DDL(DATA DEFINITION LAUGAUGE) : CREATE, ALTER, DROP - 객체 구조 변경
-- DML(DATA MANIPULATION LANGUAGE) : INSERT, UPDATE, DELETE - 값 조작
...
-- == TB1 테이블의 PK컬럼을 누군가 참조하고 있다!!
-- == -> 참조하고 있는데 이거 지워도 돼?? 안돼

-- TB1(부모)과 연결된 TB2(자식)의 FK 제약조건도 같이 삭제
