 ...
 * GROUP BY절, HAVING절에서 사용 가능(FROM 빼고 다 가능)
 */
...
-- 5번째부터 문자 검색 시작, 처음 B를 찾은 위치 조회
SELECT 'AABBCCABC', INSTR('AABBCCABC','B',5)
FROM DUAL; --8 

-- 5번째부터 문자 검색 시작, 3번째 c를 찾은 위치 조회
...
-- 모든 사원의 이메일 주소를
-- OR.KR -> GMAIL.COM으로 변경

SELECT 
	EMAIL || ' -> ' || REPLACE(EMAIL,'or.kr', 'gamil.com') AS "이메일 변경"
FROM
	EMPLOYEE ;
 ...
 * - ROUND(실수[,소수점 위치]) : 반올림
 * 
 * 1) 소수점 위치 X : 소수점 첫째 자리에서 반올림 -> 정수 형태로 값 표현
...
/* MONTH_BETWEEN(날짜, 날짜) 
 * 
 * - 두 날짜 사이의 차이나는 개월 수 반환
 * */
	
SELECT
CEIL(
MONTHS_BETWEEN(TO_DATE('2025.07.17','YYYY.MM.DD'), CURRENT_DATE))
FROM DUAL;
...
SELECT 
	CEIL (
	(SYSDATE 
	- TO_DATE('2001.03.20','YYYY.MM.DD'))/ 365 +1) "AS 한국식 나이",
...
/* <형 변환(Parsing) 함수> */
-- 문자열(CHAR, VARCHAR) <-> 숫자(NUMBER)

-- 문자열(CHAR, VARCHAR) <-> 날짜(DATE)
...
SELECT 
	'2025년 2월 20일' AS 문자열,
	TO_DATE('2025년 2월 20일'
				,'YYYY"년" MM"월" DD"일"') AS 날짜
FROM DUAL;

SELECT 
	20250220 AS 숫자,
	TO_DATE(20250220) AS 날짜 
FROM DUAL;
...
SELECT 
	EMP_NAME, TRIM(TO_CHAR(SALARY*13,'L999,999,999'))연봉
FROM 
	EMPLOYEE 
ORDER BY
	SALARY*13 DESC;
...
SELECT 
	TO_CHAR(CURRENT_DATE,'YYYY/MM/DD DAY')  AS 오늘날짜
FROM DUAL;

-- 오늘 날짜를 '2025.02.20 (목) 오후 04:27:20' 로 변환
SELECT 
	CURRENT_DATE,
	TO_CHAR(SYSDATE,'YYYY.MM.DD (DY) AM HH:MI:SS') AS 오늘날짜 
FROM DUAL;
...
SELECT 
	EMP_ID ,SALARY ,NVL(BONUS,0) AS BONUS ,NVL(SALARY*BONUS,0) AS "BONUS*SALARY"
FROM 
	EMPLOYEE ;
 ...
 * 
 * - 컬럼값이 일치하는 조건이 있으면
 * 	 해당 조건 오른쪽에 작성된 겷과가 반환되는 함수
 * 
 * - SWTICH, CASE, DEFAULT와 비슷
 */
...
-- 전화번호가 있는 사원의 수 조회(방법 1)
SELECT 
	COUNT(*)  -- NULL 포함 모든 개수 조회
FROM 	
	EMPLOYEE 
WHERE 
	PHONE IS NOT NULL;
...
-- 전화번호가 있는 사원의 수 조회(방법 2)
SELECT 
	COUNT(PHONE) -- NULL제외한 모든 개수 조회
FROM 	
	EMPLOYEE ;


-- EMPLOYEE 테이블에 존재하는 부서코드의 수 조회
-- (몇 종류의 부서코드가 존재하는지 확인)
SELECT 
	COUNT(DISTINCT DEPT_CODE) -- NULL 제외하고 중복없이 모든 개수 조회
FROM
	EMPLOYEE ;
