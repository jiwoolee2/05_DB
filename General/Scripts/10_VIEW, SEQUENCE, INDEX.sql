...
VALUES('D0','L2');
-- 가상 테이블인데 데이터 삽입이 성공?!
--> 실제로는 원본 테이블에 데이터가 삽입됨, 가상테이블에도 들어감
...
--INSERT INTO V_DCOPY2 
--VALUES('A1','메케팅1팀');
-- NULL을 ("KH18_LJW"."DEPT_COPY2"."LOCATION_ID") 
-- 안에 삽입할 수 없습니다
 ...
 *  -> VIEW는 ALTER 불가 -> CREATE OR REPALCE 이용
 * */
CREATE OR REPLACE VIEW V_DCOPY2 
AS
SELECT
...
/* 인덱스를 이용하지 않고 조회하기
 * - TEST_ID 컬럼 값이 'TEST500000'인 행 조회하기
 * */
SELECT * FROM TB_IDX_TEST 
WHERE TEST_ID = 'TEST500000'; -- 0.04S

/* 인덱스를 이용해서 조회하기
 * - TEST_NO 컬럼 값이 500000 인 행 조회하기
 * */
SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO = 500000; -- 0.025S
...
JOIN job j ~
OUTER JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID);
