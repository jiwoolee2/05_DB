...
/*  서브쿼리 유형
    ...
    - 다중행 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 여러개일 때
    	(N행 1열, WHERE절 사용 시 IN, NOT IN, > ANY, > ALL)
    	
    - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 자열된 항목수가 여러개 일 때
    	(1행 N열, WHERE절 사용 시 비교하려는 컬럼을 (A,B)로 묶어 지정)
    	
    - 다중행 다중열 서브쿼리 : 조회 결과 행 수와 열 수가 여러개일 때 
    	(N행 N열, IN 같은 연산자 + 컬럼을 (A,B)로 묶어 지정)
...
SELECT 
	E.EMP_ID ,E.EMP_NAME ,J.JOB_NAME ,J.JOB_CODE ,E.SALARY ,E.HIRE_DATE 
FROM
	EMPLOYEE E
JOIN
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
 ...
 * GROUP 함수 또는 GROUP BY에 사용된 컬럼명만 작성 가능하다!
 */
...
SELECT 
	E.EMP_ID ,E.EMP_NAME ,NVL(D.DEPT_TITLE,'없음') AS DEPT_ID 
FROM
	EMPLOYEE E
LEFT JOIN
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
SELECT 
	E.EMP_ID ,E.EMP_NAME ,NVL(D.DEPT_TITLE,'없음') AS DEPT_ID , J.JOB_NAME, '사수' AS 구분
FROM
	EMPLOYEE E
LEFT JOIN
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
SELECT 
	E.EMP_ID ,E.EMP_NAME ,NVL(D.DEPT_TITLE,'없음') AS DEPT_ID , J.JOB_NAME, '사원' AS 구분
FROM
	EMPLOYEE E
LEFT JOIN
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
SELECT 
	E.EMP_ID ,E.EMP_NAME ,NVL(D.DEPT_TITLE,'없음') AS DEPT_ID , J.JOB_NAME, 
...
--> 정렬이 되면서 순위가 지정됨
--> INLINE VIEW, ROWNUM 없이도 순위 지정이 가능하다
 ...
 * 
 * -> ROWNUM은 RESULT SET 완성 후 부여되는 가성의 컬럼으로서
...
WHERE 
	-- ROWNUM 이용
	-- ROWNUM BETWEEN 3 AND 7;  --> 1부터 시작하지 않아서 조회결과 X
	-- "순위" 이용
	"순위" BETWEEN 3 AND 7;
...
SELECT 
	...
	,MAIN.HIRE_DATE ,D.DEPT_TITLE 
FROM 
	EMPLOYEE MAIN
JOIN
	DEPARTMENT D ON (D.DEPT_ID = MAIN.DEPT_CODE)
WHERE
	MAIN.DEPT_CODE = (
	SELECT 
		SUB.DEPT_CODE 
...
WHERE
	...
	FROM 
		EMPLOYEE M
	WHERE
		TO_NUMBER(EXTRACT(YEAR FROM M.HIRE_DATE)) >= 2010
	);
...
SELECT 
	...
	MAIN.JOB_CODE ,D.DEPT_TITLE ,J.JOB_NAME 
FROM 
	EMPLOYEE MAIN
JOIN
	DEPARTMENT D ON (D.DEPT_ID = MAIN.DEPT_CODE)
JOIN
	JOB J ON (J.JOB_CODE = MAIN.JOB_CODE)
WHERE 
	(MAIN.DEPT_CODE ,MAIN.JOB_CODE) = (
		
		SELECT 
			SUB.DEPT_CODE, SUB.JOB_CODE
		FROM 
			EMPLOYEE SUB
		WHERE 
			SUB.EMP_NAME = '노옹철'
...
WHERE
	(MAIN.DEPT_CODE,MAIN.JOB_CODE) = (
		...
		FROM 
			EMPLOYEE SUB
		WHERE 
			EXTRACT(YEAR FROM SUB.HIRE_DATE) = '2010'
	);
...
-- 사번, 이름, 부서코드, 사수번호, 주민번호, 고용일
SELECT
	MAIN.EMP_ID ,MAIN.EMP_NAME ,MAIN.DEPT_CODE ,MAIN.MANAGER_ID 
	,MAIN.EMP_NO ,MAIN.HIRE_DATE 
FROM 
	EMPLOYEE MAIN
WHERE
	(MAIN.DEPT_CODE ,MAIN.MANAGER_ID) = (
		SELECT
			SUB.DEPT_CODE, SUB.MANAGER_ID
		FROM 
			EMPLOYEE SUB
		WHERE 
			SUBSTR(SUB.EMP_NO,8,1) IN ('2','4')
		AND 
			SUBSTR(SUB.EMP_NO,1,2) = '87'
	);
...
SELECT 
	...
	FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(
		CASE 
			WHEN SUBSTR(MAIN.EMP_NO,1,1) IN ('0','1') THEN '20'
			ELSE '19'
		END
	||SUBSTR(MAIN.EMP_NO,1,6),'YYYY-MM-DD'))/12) AS "만 나이",
	TO_CHAR(MAIN.SALARY * (1+NVL(MAIN.BONUS,0)) * 12,'L999,999,999') AS "보너스 포함 연봉"
...
WHERE
	FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(
		CASE 
			...
			ELSE '19'
		END
	||SUBSTR(MAIN.EMP_NO,1,6),'YYYY-MM-DD'))/12)
	= 
	(
	SELECT
		MIN(FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(
		CASE 
			WHEN SUBSTR(SUB.EMP_NO,1,1) IN ('0','1') THEN '20'
			ELSE '19'
		END
		||SUBSTR(SUB.EMP_NO,1,6),'YYYY-MM-DD'))/12))
		
	FROM
		EMPLOYEE SUB
	WHERE 
		MAIN.JOB_CODE = SUB.JOB_CODE
	)
ORDER BY
	"만 나이" DESC;
